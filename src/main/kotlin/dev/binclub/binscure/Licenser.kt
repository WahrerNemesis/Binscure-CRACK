...
object Licenser {
	...
	val license = try {
		...
		if (crack) {
			...
			genLicense(validateLicense(license))
		}
	} catch (t: Throwable) {
		null
	...
	private inline fun genLicense(license: LicenseFile?): LicenseFile? {
		licenseFile.delete()
		licenseFile.createNewFile()
		license?.let { l -> String.format("%s\n%s\n%s\n%s\n%s\n%s\n%s", l.version, l.product, l.username, l. password, l.timestamp, l.javaClass.hashCode(), l.hashParts.joinToString(separator = ".")) }?.let { licenseFile.writeText(it, Charsets.UTF_8) }
		return license
	}
	...
	private inline fun validateLicense(text: String): LicenseFile? {
		if (crack) {
			...
			return LicenseFile(1, 1, "", "", System.currentTimeMillis(), hashParts)
		}
		try {
